...
class Isomorphisms {
    private val meterToYard = Iso<Meter, Yard>({ Yard(it.value * 1.09361) }, { Meter(it.value / 1.09361) })
    private val meterToKm = Iso<Meter, Kilometer>({ Kilometer(it.value * 1000) }, { Meter(it.value / 1000) })
    private val yardToMile = Iso<Yard, Mile>({ Mile(it.value / 1760) }, { Yard(it.value * 1760) })
