...
object Transformers {
    ...
    fun <S, A> Lens<S, A>.toOptional(): Optional<S, A> = Optional(getOption = get, set = { a, s -> modify { a!! }(s) })
    ...
    infix fun <S, A, B> Lens<S, A>.compose(other: Optional<A, B>): Optional<S, B> = toOptional() compose other
